...
import 'package:gas_on_go/pages/Admin_settings.dart';
import 'package:gas_on_go/pages/admin_user_managment.dart';
import 'package:gas_on_go/pages/admin_sellers_approval.dart';
import 'package:cloud_firestore/cloud_firestore.dart';

/*    FireStore Code    */

// Shared FireStore Code:
...
Future<Map<String, int>> getThisWeekOrderStatusCounts() async {
  ...
  try {
    ...
    for (var doc in snapshot.docs) {
      ...
      print('Order ID: ${doc.id}, status: $status, cancelledAt: ${hasCancelledAt ? data['cancelledAt'] : 'none'}');
...
Future<List<Map<String, dynamic>>> fetchTopSellersWhoSoldThisWeek({int limit = 10}) async {
  ...
  results.sort((a, b) => (b['earnings'] as int).compareTo(a['earnings'] as int));
                              ...
                              children: [
                                      ...
                                      child: Column(
                                        ...
                                        children: [
                                              ...
                                              child: PieChart(
                                                PieChartData(
                                                  ...
                                                  sections: [
                                                    PieChartSectionData(
                                                      color: Colors.green,
                                                      value: counts['Processing']?.toDouble() ?? 0,
                                                      title: '',

                                                    ),
                                                    PieChartSectionData(
                                                      color: Colors.orange,
                                                      value: counts['Done']?.toDouble() ?? 0,
                                                      title: '',

                                                    ),
                                                    PieChartSectionData(
                                                      color: Colors.red,
                                                      value: counts['Cancelled']?.toDouble() ?? 0,
                                                      title: '',
                                          ...
                                          Column(
                                            crossAxisAlignment: CrossAxisAlignment.start,
                                            children: const [
                                              LegendItem(color: Colors.green, label: 'Processing'),
                                              SizedBox(height: 8),
                                              LegendItem(color: Colors.orange, label: 'Done'),
                                              SizedBox(height: 8),
                                              LegendItem(color: Colors.red, label: 'Cancelled'),
                                            ],
                                          ),
                                        ],
                                      ),
                                ...
                                FutureBuilder<List<int>>(
                                  ...
                                  builder: (context, snapshot) {
                                    ...
                                    final maxYValue = (data.reduce((a, b) => a > b ? a : b) + 3).toDouble();
                                    final maxY = maxYValue < 15 ? 15.0 : maxYValue;
                        ...
                        Expanded(
                          child: Container(
                              ...
                              child:  Column(
                                children: [
                                  SizedBox(height: 8),
                                  Text('Today Orders', style: TextStyle(fontSize: 18)),
                                  SizedBox(height: 6),
                                  FutureBuilder<int>(
                                    /*    FireStore Code    */
                        ...
                        Expanded(
                          child: Container(
                              ...
                              child: Column(
                                children: [
                                  SizedBox(height: 8),
                                  Text('Today Total Revenue', style: TextStyle(fontSize: 18)),
                                  SizedBox(height: 6),
                                  FutureBuilder<int>(
                                    future: countTodayOrders(),
                                    builder: (context, snapshot) {
                        ...
                        FutureBuilder<List<Map<String, dynamic>>>(
                          ...
                          builder: (context, snapshot) {
                            if (snapshot.connectionState == ConnectionState.waiting) {
                              ...
                              return const Text('Failed to load sellers.', style: TextStyle(color: Colors.white));
                            } else if (!snapshot.hasData || snapshot.data!.isEmpty) {
                              return const Text('No sellers found.', style: TextStyle(color: Colors.white));
                            }
