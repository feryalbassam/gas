...
class _SellerDashboardPageState extends State<SellerDashboardPage> {
  ...
  Future<void> _acceptOrder(
    ...
    try {
      ...
      setState(() {
        hasActiveOrder = true;
        _acceptedOrderId = orderId;
      });

      _listenToOrderStatusChanges(orderId); // ✅ Listen for changes
  ...
  void _listenToOrderStatusChanges(String orderId) {
    FirebaseFirestore.instance
      ...
      if (doc.exists) {
        ...
        if (status == 'completed' || status == 'cancelled') {
          ...
          ScaffoldMessenger.of(context).showSnackBar(
            SnackBar(content: Text("✅ Order $orderId is now $status.")),
          );
        }
      }
    });
                ...
                child: ListTile(
                  ...
                  trailing: ElevatedButton(
                    onPressed: hasActiveOrder
                        ? null
                        : () => _acceptOrder(context, order.id, userId),
                    style: ElevatedButton.styleFrom(
                      backgroundColor: accentColor,
                      padding: const EdgeInsets.symmetric(
                          horizontal: 12, vertical: 8),
